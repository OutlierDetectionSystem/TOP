...
public class GenerateLocalFSViolation {

	public static enum Counters {
		fsCompTime, fslengthSum, fsNum, startLen, numCountSL, candidateCount;
	}
	...
	public static class DDReducer extends Reducer<IntWritable, Text, IntWritable, Text> {
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException {
			...
			for (Text oneValue : values) {
				...
				Configuration conf = context.getConfiguration();
				String outputOutlierPath = conf.get(SQConfig.strLocalFSOutputLTS) + "-" + conf.getInt(SQConfig.strLocalSupport, 10)
				...
				outputKey.set(seqId);
				try {
					mos.write(outputKey, outputValue, outputOutlierPath + "/LocalFS-");
				...
				for(Map.Entry<String, Set<String>> tempViolation: violations.entrySet()){
					for(String str: tempViolation.getValue()){
						outputResult += tempViolation.getKey() + "|" + str + "\t";
					}
				}
				if (outputResult.length() > 0)
					outputResult = outputResult.substring(0, outputResult.length() - 1);
				outputValue.set(outputResult);
				String outputLocalViolationPath = conf.get(SQConfig.strLocalViolationOutput) + "-" + conf.getInt(SQConfig.strLocalSupport, 10)
						+ "-" + conf.getInt(SQConfig.strEventGap, 1) + "-" + conf.getInt(SQConfig.strSeqGap, 1);
				try {
					mos.write(outputKey, outputValue, outputLocalViolationPath + "/LocalViolation-");
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public void run(String[] args) throws Exception {
		...
		/** set job parameter */
		Job job = Job.getInstance(conf, "Generate Local Frequent Sequences--- With Violation");

		job.setJarByClass(GenerateLocalFSViolation.class);
...
//		System.out.println(
//				"sequence gap \t event gap \t local support \t lts support \t total time \t end to end \t Num FS \t  Avg length");
	...
	public static void main(String[] args) throws Exception {
		GenerateLocalFSViolation generateLocalFS = new GenerateLocalFSViolation();
