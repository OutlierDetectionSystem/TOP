...
public class DetectGlobalFS {

	public static enum Counters {
		numGlobalFreqSeq, sumFreqSeqLength;
	}
...
//								this.metaDataMapping.put(splitsStr[0], "(" + splitsStr[1] + ")" + splitsStr[2]);
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException {
			...
			for (String str : gbFreqSeqs.keySet()) {
				if (gbFreqSeqs.get(str) >= this.minSupportGlobal) {
					finalGBFreqSeq.add(str);
				}
			}
			context.getCounter(Counters.numGlobalFreqSeq).increment(finalGBFreqSeq.size());
			for(String str: finalGBFreqSeq){
				context.getCounter(Counters.sumFreqSeqLength).increment(str.split(",").length);
			}
			// detect outliers by traversing the dataset again

			for (String curStr : finalGBFreqSeq) {
				try {
					String outputMeta = "";
					...
					context.write(NullWritable.get(), new Text("Frequent Sequence: " + curStr));
					context.write(NullWritable.get(), new Text(outputMeta + "\n"));
			...
			Configuration conf = context.getConfiguration();
			String outputOutlierPath = conf.get(SQConfig.strGlobalFSOutputForViolation) + "-" + conf.getInt(SQConfig.strLocalSupport, 10)
					+ "-" + conf.getInt(SQConfig.strEventGap, 1) + "-" + conf.getInt(SQConfig.strSeqGap, 1);
			// output for violations
			for(String curStr: finalGBFreqSeq){
				try {
					mos.write(NullWritable.get(), new Text(curStr), outputOutlierPath + "/GlobalFS-");
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
	...
	public void run(String[] args) throws Exception {
		...
		job.setJarByClass(DetectGlobalFS.class);
		...
		FileSystem fs = FileSystem.get(conf);
		String outputPath = conf.get(SQConfig.strGlobalFSOutput) + "-" + conf.getInt(SQConfig.strLocalSupport, 10)
				+ "-" + conf.getInt(SQConfig.strGlobalSupport, 10) + "-" + conf.getInt(SQConfig.strEventGap, 1) + "-" + conf.getInt(SQConfig.strSeqGap, 1);
		...
		Counter c1 = cn.findCounter(Counters.numGlobalFreqSeq);
		Counter c2 = cn.findCounter(Counters.sumFreqSeqLength);
		System.out.print(c1.getValue() + "\t" + c2.getValue()*1.0/c1.getValue() + "\t");
		
//		LOG.info("Test if can output to file!");
	}

	public static void main(String[] args) throws Exception {
		DetectGlobalFS generateLocalFS = new DetectGlobalFS();
