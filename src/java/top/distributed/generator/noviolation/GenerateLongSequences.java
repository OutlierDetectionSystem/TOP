...
public class GenerateLongSequences {
	...
	public static class DDReducer extends Reducer<IntWritable, Text, NullWritable, Text> {
		...
		private ArrayList<String> outlierSequences;
		...
		public void setup(Context context) {
			...
			this.numFreqPatterns = conf.getInt(SQConfig.strNumFreqPatterns, 1000);
			this.numPatterns = conf.getInt(SQConfig.strNumPatterns, 5000);
					...
					for (int i = 0; i < stats.length; ++i) {
						if (!stats[i].isDirectory() && stats[i].getPath().toString().contains("FrequentPatterns.csv")) {
							...
							System.out.println("Size of pattern candidates: " + this.patternCandidates.size());
								...
								&& stats[i].getPath().toString().contains("outlierPatterns.csv")) {
							...
							String line;
							while ((line = currentReader.readLine()) != null) {
								this.outlierSequences.add(line);
							}
							System.out.println("Size of outliers: " + this.outlierSequences.size());
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException {
			...
			int numOutlierPerPartition = this.outlierSequences.size() / this.numPartitions;
			ArrayList<String> tempOutlierList = new ArrayList<String>(this.outlierSequences
					.subList(partitionNumber * numOutlierPerPartition, (partitionNumber + 1) * numOutlierPerPartition));
			context.getCounter(Counters.outlierCount).increment(tempOutlierList.size());
			...
			gd.addOutliers(tempOutlierList);
			gd.addFrequentPatterns(tempFSList);
			gd.addMoreFrequentPatterns(tempFSList);
			gd.addPatternCandidates(patternCandidates, sequenceLength);
			gd.outputDeviceDataToFile(context, 1 - this.probability);
