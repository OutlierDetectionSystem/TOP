        ...
        public void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException {
            ...
            for (Text oneValue : values) {
                try {
                    ...
                    SingleSequenceMiningTS localFS = new SingleSequenceMiningTS(
                            inputSequence, localParameterSpace);
                    Set<String> tempLocalFS = localFS.findFreqSeqInOneString(globalFreqElements);
                    long timeCost = (System.currentTimeMillis() - start);
                    context.getCounter(top.distributed.lts.explanation.GenerateLocalFSViolationWithTS.Counters.fsCompTime).increment(timeCost);
                    // output local frequent sequences
                    Configuration conf = context.getConfiguration();
                    String outputOutlierPath = conf.get(SQConfig.strLocalFSOutputSTL) + "-"
                    ...
                    for (String str : tempLocalFS) {
                        ...
                        context.getCounter(top.distributed.lts.explanation.GenerateLocalFSViolationWithTS.Counters.fsNum).increment(1);
                        context.getCounter(top.distributed.lts.explanation.GenerateLocalFSViolationWithTS.Counters.fslengthSum).increment(subs.length);
                    }
...
//                    }
    ...
    public void run(String[] args) throws Exception {
        ...
        /** set job parameter */
        Job job = Job.getInstance(conf, "Generate Local Frequent Sequences using Max --- With Violation and time stamp");

        job.setJarByClass(GenerateLocalFSViolationWithTS.class);
        job.setMapperClass(GenerateLocalFSViolationWithTS.DDMapper.class);
        job.setReducerClass(GenerateLocalFSViolationWithTS.DDReducer.class);
