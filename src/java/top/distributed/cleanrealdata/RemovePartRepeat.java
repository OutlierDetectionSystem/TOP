...
public class RemovePartRepeat {
	...
	public static class DDReducer extends Reducer<IntWritable, Text, NullWritable, Text> {
		public void reduce(IntWritable key, Iterable<Text> values, Context context)
			...
			for (Text oneValue : values) {
				...
				double portion = countRepeat * 1.0 / subStr.length;
				if (portion > 0.1) {
					String outputValues = "";
					for (String sub : subStr) {
						if (!sub.equals(previousStr)) {
					...
					context.write(NullWritable.get(), new Text(seqNum + "\t" + deviceId + "\t" + outputValues));
				} else {
					context.write(NullWritable.get(), oneValue);
				}
			}
		} // end
			// reduce
	...
	public void run(String[] args) throws Exception {
		...
		job.setJarByClass(RemovePartRepeat.class);
	...
	public static void main(String[] args) throws Exception {
		RemovePartRepeat cleanData = new RemovePartRepeat();
		cleanData.run(args);
	}
}
